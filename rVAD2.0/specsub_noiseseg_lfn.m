function [ss,gg,tt,ff,zo]=specsub_noiseseg_lfn(si,fsz,noise_seg,pv01,pp)
...
if isstruct(fsz)
    if ne>0
        ...
        if size(noise_seg)<nr; noise_seg(nr) = 0; end % to match the size with yp. 
        [dp,ze]=estnoisem_noiseseg(yp,noise_seg,pv01,ze);       % estimate the noise using minimum statistics
    end
    ssv=fsz.ssv;
else
    if ne>0
        ...
        if size(noise_seg,1)<nr; noise_seg(nr) = 0; end % to match the size with yp.
        [dp,ze]=estnoisem_noiseseg(yp,noise_seg,pv01,tinc,qp);	% estimate the noise using minimum statistics
    end
...
if ~nr                                  % no data frames
    ...
    %--low frequency noise 
    for i=1:nr    
        if sum(yp(i,1:7),2)>sum(yp(i,:),2)/2   %% 7 frequecy bins condition
            yp(i,1:7)=0;
            out_p(i,1:7)=0;
            out(i,1:7)=0+0i;
        end
    ...
    for i=11:nr-10
        if out_pf(i)<out_dpft(i) && sum(out_smth(i-10:i+10))<5
            out(i,:)=0+0i;            
        end
    end
    ...
    se=(irfft((out).',nf).').*repmat(w,nr,1);   % inverse dft and apply output window
   
    ss=zeros(ni*(nr+no-1),no);                      % space for overlapped output speech
    ss(1:ni*(no-1),end)=ssv;
    for i=1:no   % e.g. no=2, ni=128, nr=number of frames
